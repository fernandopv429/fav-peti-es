...
export default function Sidebar({ onClose }) {
  ...
  return (
    <div className="h-full flex flex-col bg-[#0c1120] border-r border-white/[0.06]">
      {/* Logo */}
      <div className="h-16 flex items-center justify-between px-5 border-b border-white/[0.06]">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 rounded-xl flex items-center justify-center shadow-lg shadow-amber-500/20" style={{ background: "linear-gradient(135deg, #f59e0b, #d97706)" }}>
            <Scale className="w-4 h-4 text-white" />
          </div>
          <div>
            <h1 className="font-bold text-sm tracking-wide text-white">FAV Petições</h1>
            <p className="text-[10px] text-white/30">Fernando Vieira Advogados</p>
          </div>
        </div>
      ...
      <nav className="flex-1 px-3 py-4 overflow-y-auto space-y-5">
        {NAV.map((section) => (
          <div key={section.group}>
            <p className="text-[10px] font-bold uppercase tracking-widest text-white/20 px-3 mb-2">{section.group}</p>
                ...
                return (
                  ...
                  >
                    <div className={`w-7 h-7 rounded-lg flex items-center justify-center transition-all ${
                      isActive ? "bg-amber-500 shadow-md shadow-amber-500/40" : "bg-white/[0.05] group-hover:bg-white/[0.08]"
                    }`}>
      ...
      <div className="p-3 border-t border-white/[0.06]">
        <button
          ...
          className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm text-white/25 hover:text-white/60 hover:bg-white/[0.05] transition-all w-full"
        >
